      ...
      if (mode === 'production') {
        ...
        if (this.isGithubActions) {
          ...
          if (this.prefix) {
            ...
            // output对象中的publicPath属性
            const publicPath = outputOptions.publicPath;

            // 用于为静态资源（如图像、样式表、JavaScript 文件等）设置 URL 前缀
            // 这在将应用部署到自定义域名或 CDN 上时特别有用，因为它允许您将静态资源存储在不同的位置
            // 修改publicPath
            outputOptions.publicPath = typeof publicPath === 'string' && publicPath.startsWith(`/${this.prefix}`) ? outputOptions.publicPath : `/${this.prefix}/${outputOptions.publicPath}/`.replace(/\/\//g, '/');
          }
        }
      }
    });
